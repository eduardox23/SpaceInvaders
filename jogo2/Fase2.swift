...
class Fase2: SKScene, SKPhysicsContactDelegate{
    var linhasInimigos = 8
    var colunasInimigos = 6
    var countInimigos = 8 * 6
    ...
    func killEnemy(inimigo: SKNode) {
        ...
        if(countInimigos == 0){
            var gameScene = Fase3_Intro(size: self.size)
    ...
    func criaTiroInimigo(inimigo: SKSpriteNode){
        ...
        var duracao = NSTimeInterval(2 + 15 * Float(arc4random()) / Float(UINT32_MAX))
        ...
        var teste = SKAction.sequence([SKAction.waitForDuration(duracao-2), SKAction.runBlock {
            self.addChild(tiroInimigo)
            }])
        self.runAction(teste)
    ...
    override func didMoveToView(view: SKView) {
        ...
        for(var i2=0; i2<linhasInimigos; i2++){
            ...
            for(var i=0; i<colunasInimigos; i++){
                ...
                var acaoSequencia = SKAction.sequence([acaoMoveDireita,acaoMoveBaixo,acaoMoveEsquerda,acaoMoveBaixo, SKAction.runBlock {
                    duracaoAcaoMoveHorizontal += 1
                    }
                    ])
                var acaoHorizontalLoop = SKAction.repeatActionForever(acaoSequencia)
                
                
                //inimigo.physicsBody = SKPhysicsBody(texture: inimigo.texture, size: inimigo.size)
